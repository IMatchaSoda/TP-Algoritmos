			...
			while (!salir){
				...
				switch(opcionL){
					...
					case "a":
						...
						Obrero ob1= new Obrero(nombre,apellido,cargo,sueldo,legajo,dni);
						e1.ListaGrupos[id].Add(ob1);
						break;
					case "b":
					Console.Clear();
					...
					switch(op2)
					{
						...
						{
							if(e1.ListaObras[i].Estado_Avance> 50)
							{
						...
						for(int i=0; i<e1.listaObras.Count;i++)
						{
							if(e1.ListaObras[i].Estado_Avance==100)
							{
							Console.WriteLine(e1.ListaObras[i]);
							}
						}
						...
						for(int j=0; j<e1.ListaGrupos.Count;j++){
							foreach(Obrero o in e1.ListaGrupos[j]){
								if(o.Cargo=="jefe")
								{
									o.imprimir();
								}
                    ...
                    case "d":
						...
						foreach(Grupo_Obrero g in e1.ListaGrupos){
							if(g.IDobra==null){
								disponible=true;
								g.IDobra=idob;
								Jefe_Obrero j1= new Jefe_Obrero(bono,g,nombre,apellido,cargo,sueldo,legajo,dni);
								break;
							}
							if(!disponible){
